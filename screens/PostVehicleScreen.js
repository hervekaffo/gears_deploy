...
const emptySeasonDraft = { label: '', start: '', end: '', rate: '' };
...
function mapExistingPhotos(vehicle) {
  if (!vehicle) return [];
  const photos = Array.isArray(vehicle.media?.photos) ? vehicle.media.photos : [];
  if (photos.length) {
...
function mapBlockedRanges(vehicle) {
  if (!vehicle?.availabilityOverrides) return [];
...
export default function PostVehicleScreen({ navigation, route }) {
  const { addVehicle, updateVehicle, deleteVehicle } = useContext(VehicleContext);

  const initialVehicle = route?.params?.vehicle || null;
  const mode = route?.params?.mode || (initialVehicle ? 'edit' : 'create');
  const isEditMode = mode === 'edit';

  const currentYear = useMemo(() => new Date().getFullYear().toString(), []);

  const [title, setTitle] = useState(initialVehicle?.title || '');
  const [vehicleType, setVehicleType] = useState(initialVehicle?.vehicleType || '');
  const [brand, setBrand] = useState(initialVehicle?.brand || '');
  const [model, setModel] = useState(initialVehicle?.model || '');
  const [year, setYear] = useState(
    initialVehicle?.year != null ? String(initialVehicle.year) : currentYear
  );
  const [powerType, setPowerType] = useState(
    initialVehicle?.powerType || (initialVehicle?.electric ? 'Electric' : POWER_TYPES[0])
  );
  const [capacity, setCapacity] = useState(
    initialVehicle?.capacity != null
      ? String(initialVehicle.capacity)
      : initialVehicle?.seats != null
      ? String(initialVehicle.seats)
      : ''
  );
  const [description, setDescription] = useState(initialVehicle?.description || '');

  const [pickupOptions, setPickupOptions] = useState(() => {
    const options = Array.isArray(initialVehicle?.fulfillmentOptions)
      ? initialVehicle.fulfillmentOptions
      : [];
    const lower = options.map(opt => String(opt || '').toLowerCase());
    return {
      pickup: lower.includes('pick-up') || lower.includes('pickup'),
      delivery: lower.includes('delivery'),
      trailer: lower.some(opt => opt.includes('trailer')),
    };
  });

  const [features, setFeatures] = useState(
    Array.isArray(initialVehicle?.features) ? initialVehicle.features : []
  );

  const onboarding = initialVehicle?.onboarding || {};
  ...
  const [photos, setPhotos] = useState(() => mapExistingPhotos(initialVehicle));
  const [videoClip, setVideoClip] = useState(() => mapExistingVideo(initialVehicle));

  const pricing = initialVehicle?.pricing || {};
  const [baseRate, setBaseRate] = useState(
    pricing.baseDailyRate != null
      ? String(pricing.baseDailyRate)
      : initialVehicle?.dailyRate != null
      ? String(initialVehicle.dailyRate)
      : ''
  );
  ...
  const [blockedRanges, setBlockedRanges] = useState(() => mapBlockedRanges(initialVehicle));
  ...
  const [securityDeposit, setSecurityDeposit] = useState(
    initialVehicle?.securityDeposit != null ? String(initialVehicle.securityDeposit) : ''
  );
  const [city, setCity] = useState(initialVehicle?.city || '');
  const [locationQuery, setLocationQuery] = useState(initialVehicle?.location?.address || '');
  const [location, setLocation] = useState(
    initialVehicle?.location
      ? {
          lat: initialVehicle.location.latitude,
          lng: initialVehicle.location.longitude,
          address: initialVehicle.location.address,
        }
      : null
  );
  ...
  function addSeasonalAdjustment() {
    ...
    setSeasonalAdjustments(prev => [
      ...prev,
      { id: `${Date.now()}`, label: label.trim(), start: start.trim(), end: end.trim(), rate: parsedRate },
    ]);
  ...
  async function handleSubmit() {
    ...
    if (!photos.length) {
      Alert.alert('Photos needed', 'Add at least one photo to showcase your vehicle.');
      return;
    }
    ...
    try {
      ...
      const ownerId = initialVehicle?.ownerId || auth.currentUser?.uid;
      ...
      if (isEditMode) {
        await updateVehicle(initialVehicle.id, vehicleData);
        setSaving(false);
        ...
        setSaving(false);
        Alert.alert('Listing created', 'Your vehicle has been saved for review.', [
          {
            text: 'My listings',
            onPress: () => navigation.navigate('Profile', { screen: 'MyListings' }),
          },
          { text: 'OK' },
        ]);
      }
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteVehicle(initialVehicle.id);
